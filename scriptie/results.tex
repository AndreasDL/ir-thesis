...
\hfill
  \begin{subfigure}[b]{.4\textwidth}
    ...
    \caption{Each color and its corresponding EEG channel.\label{channellegend}}
  \end{subfigure}
\end{figure}
...
\end{tabular}
\caption{The region for each EEG channel\label{my-label}}
\end{table}

These Region are also depicted in Figure \ref{regions}.
\mijnfiguur{width=.8\textwidth}{regions}{All EEG channels were grouped in 4 regions: left-front (blue), right-front (red), left-back(black), right-back (purple) and midline (yellow).} %TODO cleanup
The resulting distribution are shown in figure 
...
Looking at the pie plots, one can see that there is no clear region used. It seems that for both valence and arousal, power value of all channels might be of importance. Another explanation might be that the selected features are overfitting. With as little as 30 samples, to rank the features, on can easily pick a feature that works well be coincidence. 

%DCAU and RCAU FEATURES! 
