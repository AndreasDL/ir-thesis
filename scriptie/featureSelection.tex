\chapter{Methods}
%{\samenvatting TODO %TODO }

\section{Emotion recognition in the literature}
...
The input for a CSP filter is a set of N labelled samples $E_j (j=1...N)$, with dimension $N_ch x T_j$, with $N_ch$ being the number of EEG channels and $T*j$ the number of samples in a single trial\citep{CSPSeba}.
...
Linear Discriminant Analysis (LDA)\nomenclature{LDA}{Linear Disciminant Analysis}, is a machine learning technique often used in combination with CSP\cite{ErrorPotentials,svmldacomp,currTrends}. LDA looks for a projection of the data where the data is linearly separable, as shown in Figure \ref{lda}.
...
$OOB_init = AVG(OOB) - STD(OOB)$
\end{center}
The next part is different, now a feature is introduced in each iteration. When the average OOB score of the feature is better, the feature is added to the features set, otherwise it is neglected. The different between step2-interpretation and step2-prediction is that here single features are added to the feature set, while step2-interpretation always takes the feature set containing all features with higher importance than the lastly added feature. step2-prediction on the other hand is able to select a distinct set of features out of the results from step1.

\npar
In the end the paper notes several observations, the step2-prediction method provides better OOB scores using fewer features. Additionally they mention that highly correlated features might confuse the algorithm, as correlated features have lower importances.
...
\begin{center}
$frac_band = \frac{power_band}{power_total}$
\end{center}
