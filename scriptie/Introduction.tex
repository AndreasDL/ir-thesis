...
As mentioned before, EEG is used in this thesis, because it is a convenient method. Alternative methods to measure brain activity are:
\begin{itemize}
...
$\vdots$                        & $\vdots$
\end{tabular}
\caption{total area of a house and the corresponding asking price.\label{mlexampleTable}}
\end{table}
\clearpage

One possible way of predicting the asking price of a house is machine learning. Machine learning works in several steps, first you need to train your machine learning algorithm with a list of asking prices and the corresponding area of the house. This process is called training or fitting and gives the machine learning component an idea to what the price is for a house with a certain area.
Once trained, the machine learning algorithm will look like Figure \ref{mlexample}. The black dots represent data points, while the blue line is the predicted price for those points. As you can see the asking price is simply determined by the total area of the house multiplied by some constant.

\mijnfiguur{width=0.9\textwidth}{mlexample}{The price of a house is determined by its total area.}

As you can see there is sometimes a big difference between the predicted value and the actual value. This is due to the fact that the area of the house is only one feature that determines the price. Other features, like the number of bedrooms or the location of the house, were not taken into consideration. Adding additional features, gives more insight into the data, e.g. a house with 5 bedrooms is more expensive than a house with only 3 bedrooms. Having more features is thus likely to improve the performance of the machine learning algorithm.
